...
* `input_video`: 视频到视频生成的源视频帧列表（PIL 图像）。源视频先被编码，再按 `denoising_strength` 加噪，然后从该噪声水平开始去噪，因此结果会保留源视频的构图与运动，同时由 prompt 主导画面与声音。帧列表必须已经是 24fps；帧会被缩放到目标画幅并截断到 `num_frames`，不足时重复最后一帧补齐。
* `input_audio`: 以同样方式作为音频分支起点的源波形 `Tensor[C, L]`。它会被转为立体声、重采样到音频 VAE 的采样率，并裁剪或补齐到视频时长。留空时音频从纯噪声开始生成，而此时的调度已经为视频缩短过，`denoising_strength` 较低时留给音频收敛的空间很少，因此源视频自带音轨时建议一并传入。`read_video_audio`（见下文 `retake_audio`）可以从同一个文件中读出时间对齐的帧与波形。
* `input_audio_sample_rate`: `input_audio` 的采样率，默认为 32000。
* `denoising_strength`: 加噪后的输入从调度的哪个位置开始去噪，默认为 1.0（完全忽略输入，即纯文生视频）。它作用在 `flow_shift` **之前**的调度位置上，而默认 shift 为 12 时这条曲线非常陡，因此可用取值远小于其他模型：1.0 / 0.5 / 0.3 / 0.2 / 0.15 / 0.1 对应视频 latent 上的噪声水平分别约为 1.00 / 0.92 / 0.84 / 0.75 / 0.68 / 0.57。建议从 0.1-0.2 开始尝试，超过 0.3 左右源视频基本已被覆盖。推理步数不随之减少，因此低强度的耗时与完整生成相同。视频与音频共用这一个取值：训练时两条调度按相同的步数下标配对，二者的起点必须保持对齐才不会偏离训练分布。
...
如果显存不足，请开启[显存管理](../Pipeline_Usage/VRAM_management.md)，我们在示例代码中提供了每个模型推荐的低显存配置，详见前文“模型总览”中的表格。此外我们还提供了 NF4 量化版本的权重，可进一步降低显存需求，对应脚本同样见“模型总览”表格。我们同时支持 Comfy-Org 发布的 int8 量化权重，通过 comfy-kitchen 后端加载，需要安装 `pip install "diffsynth[quant]"`。
