...
#   --dataset_metadata_path /mnt/nas3/sunyuzework/Diffutoon-2/data/xinhaicheng_39_t2va.jsonl \
...
#   --output_path "./models/train/MiniMax-H3-ToonLoRA-split-cache-480p" \
#   --lora_base_model "dit" \
...
# T2VA - stage 2 (train)
accelerate launch examples/minimax_h3/model_training/train.py \
  --dataset_base_path ./models/train/MiniMax-H3-ToonLoRA-split-cache-480p \
  --data_file_keys "video,input_audio" \
  --extra_inputs "input_audio" \
  --max_pixels 399360 \
  --num_frames 39 \
  ...
  --save_steps 200 \
  --remove_prefix_in_ckpt "pipe.dit." \
  --output_path "./models/train/MiniMax-H3-ToonLoRA-split" \
  --lora_base_model "dit" \
  --lora_target_modules "attn.qkv_proj,attn.out_proj,mlp.fc1,mlp.fc2" \
